...
const skill = {
	...
	th_jinjing: {
		...
		filter: function (event, player) {
			return game.hasPlayer((c) => c != player && !c.hasSkill('th_jinjing2'));
		},
		content: function () {
	...
	th_ruyi: {
		...
		filter: function (event, player) {
			...
			else if (['phase', 'enterGame'].includes(event.name)) return game.phaseNumber == 0;
			return false;
		},
		content: function () {
			if (trigger.name == 'phase' || trigger.name == 'enterGame') {
				if (!lib.inpile.includes('th_ruyijingubang')) {
			...
			ban: {
				mod: {
					playerEnabled: function (card, player, target) {
						if (["basic", "trick"].includes(get.type(card)) && get.distance(player, target) > 1) {
							return false;
						}
					},
				},
	...
	cxyXiaoXiong: {
		...
		filter: function (event, player) {
			return event.player != player && _status.currentPhase != player && ["basic", "trick"].includes(get.type(event.card));
		},
		forced: true,
		content: async function (event, trigger, player) {
			await player.draw();
	...
	dxjtz_qiaoshi: {
		...
		check: function (event, player) {
			return game.hasPlayer(function (current) {
				...
				var targetsExceptCurrent = game.filterPlayer(function (cur2) {
					return cur2 != current && event.targets.includes(cur2);
				});
		...
		content: async function (event, trigger, player) {
			...
			const firstResult = await player.chooseTarget(1, true, '请选择一名其他角色成为此牌的使用者', function (card, player, target) {
				...
				.set('ai', function (target) {
					...
					const targetsExceptCurrent = game.filterPlayer(function (cur2) {
						return cur2 != target && tri.targets.includes(cur2);
					});
					if (!tri.targets.includes(player)) {
			...
			if (!trigger.targets || !trigger.targets.includes(trigger.player)) {
				return;
			}
			...
			const secondResult = await event.cplayer.chooseTarget(1, true, "请为此牌选择一个额外目标。", function (card, player, target) {
				return !tri2.targets.includes(target);
			})
				.set('ai', function (target) {
			...
			const thirdResult = await event.cplayer.chooseTarget(1, true, "请选择对" + get.translation(trigger.player) + "造成伤害的角色。", function (card, player, target) {
				return tri3.targets.includes(target);
			})
				.set('ai', function (target) {
	...
	dpoioned: {
		...
		content: async function (event, trigger, player) {
			...
			if (compareResult.num1 == compareResult.num2) {
				const minc = Math.max(1, Math.floor(compareResult.num1 / 2));
				if (player.countMark("dpoioned") < 7) {
					player.addMark("dpoioned", 1);
					player.syncStorage("dpoioned");
				}
			...
			if (compareResult.num1 < compareResult.num2) {
				...
				const damage1 = Math.max(1, Math.floor(compareResult.num1 / 2));
				await player.loseHp(damage1);
			}
			if (compareResult.num1 > compareResult.num2) {
				player.removeMark("dpoioned", 1);
				const damage2 = Math.max(1, Math.floor(compareResult.num2 / 2));
				await trigger.player.loseHp(damage2);
			}
		},
		derivation: "selfbang",
